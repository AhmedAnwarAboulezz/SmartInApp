...
@Component({
  selector: 'app-approve-full-day-permission-request',
  templateUrl: './approve-full-day-permission-request.page.html',
  styleUrls: ['./approve-full-day-permission-request.page.scss'],
})
export class ApproveFullDayPermissionRequestPage  implements OnInit {
  model: any = {};
  id: any;
  segmentValue = true;
  ...
  getDocument(fileName : string){
   ...
   this.service.downloadfile(fileName, "EmployeeFullDayPermissions") 
   .subscribe(response => {
 ...
 downLoadFile(data: any, type: string) {
  ...
  getRequestById() {
    this.subRequests = this.service.getRequestsById2(this.id).subscribe((res: any) => {
      ...
      this.approveDto.startDate = res.employeeFullDayPermissionDto.startDate;
  ...
  approve(isApprove: boolean): void {
    ...
    this.service.approveOrreject(this.approveDto).subscribe((res: any) => {
      ...
      if (res == null) {
        ...
        this.router.navigate(['/home/requests/manager-full-day/']);
      }
    }, error => {
      this.base.hideLoader();
