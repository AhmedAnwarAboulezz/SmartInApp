...
@Component({
  ...
  templateUrl: './approve-leave-request.page.html',
  styleUrls: ['./approve-leave-request.page.scss'],
})
export class ApproveLeaveRequestPage  implements OnInit {
  model: any = {};
  id: any;
  segmentValue = true;
  ...
  getRequestById() {
    this.subRequests = this.service.getRequestsById2(this.id).subscribe((res: any) => {
      ...
      this.approveDto.startDate = res.employeeLeaveDto.startDate;
  ...
  approve(isApprove: boolean): void {
    ...
    this.service.approveOrreject(this.approveDto).subscribe((res: any) => {
      ...
      if (res == null) {
        ...
        this.router.navigate(['/home/requests/manager-leave/']);
      }
    }, error => {
      this.base.hideLoader();
